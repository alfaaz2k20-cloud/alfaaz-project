...
def _get_config(db: Session) -> DBExhibitionConfig:
...
@router.post("/apply")
def apply_for_exhibition(
    data: ExhibitionApplicationCreate,
    ...
    config = _get_config(db)
    current_cycle = config.title  # e.g. "Kaamil III"

    # Block only if they already have a PENDING app in THIS cycle
    ...
    # Also block if they're already APPROVED / CONFIRMED in this cycle
    ...
    application = DBExhibitionApplication(
        ...
        exhibition_cycle=current_cycle,   # <-- stamp it
...
def get_my_exhibition_status(db: Session = Depends(get_db), user=Depends(require_auth)):
    ...
    # Always look up the user's application for the CURRENT cycle only
...
def finalize_exhibition_registration(
...
# Keep the old alias so nothing breaks if it's called directly
